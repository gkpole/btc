...
TOKEN = '5906923133:AAEgRXD-tZyZ9ZShU-RP7thS2GTsmNkSp1A' 
bot = Bot(token=TOKEN)
...
async def start(message, state: FSMContext):
    ...
    await bot.send_message(message.chat.id, 'бот является примером для канала портфолио пользователя @noziss')
